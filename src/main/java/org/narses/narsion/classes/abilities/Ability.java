...
public enum Ability {
    ...
    public static Ability[] getAbilities(String className) {
        ...
        for (final Ability ability : values()) {
            if (ability.playerClassName.equals(className)) {
                list.add(ability);
            }
        }

        return list.toArray(Ability[]::new);
    }

    private final Consumer<NarsionPlayer> abilityCallback;
    private final int cooldown;
    private final String playerClassName;

    Ability(String playerClassName, int cooldown /*Consumer<NarsionPlayer> abilityCallback*/) {
        this.playerClassName = playerClassName;
        this.cooldown = cooldown;
        this.abilityCallback = null; // TODO: Add all ability callbacks back
    }

    /**
     * Activates the ability with the player argument
     *
     * @param player
     */
    public void activate(NarsionPlayer player) {
        if (this.abilityCallback != null) {
            this.abilityCallback.accept(player);
        }
    }
