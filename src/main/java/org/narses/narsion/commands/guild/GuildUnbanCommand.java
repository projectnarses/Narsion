...
class GuildUnbanCommand extends GuildRequiredCommand {

    public GuildUnbanCommand(@NotNull NarsionServer server) {
        super(server, "unban");
    ...
    private void handlePlayer(CommandSender sender, CommandContext context) {
        ...
        manager.unbanMemberFromGroup(playerToBanUuid, guild.getUuid());
        ...
        if (sender instanceof Player player) {
            ...
            if (!kickRank.isLowerThan(rank)) {
                sender.sendMessage("You can't unban a player with a rank higher or equal to yours.");
            }

            playerToBan.sendMessage("You have been unbanned from the guild " + guild.getName() + " by " + player.getUsername());
        } else {
            playerToBan.sendMessage("You have been unbanned from the guild " + guild.getName() + ".");
        }
    }
}
